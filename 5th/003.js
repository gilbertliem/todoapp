...
function multiplyTheOdds(arrOperand1, arrOperand2) {
    ...
    if (arrOperand1.length < arrOperand2.length) {
      const diff = arrOperand2.length - arrOperand1.length;
      for (let i = 0; i < diff; i++) {
        arrOperand1.push(0);
      }
    } else if (arrOperand1.length > arrOperand2.length) {
      const diff = arrOperand1.length - arrOperand2.length;
      for (let i = 0; i < diff; i++) {
        arrOperand2.push(0);
      }
    }
    ...
    for (let i = 0; i < arrOperand1.length; i++) {
      arrOperand1[i] *= arrOperand2[i];
      if (arrOperand1[i] % 2 !== 0) {
        returnNum *= arrOperand1[i];
      }
    }

    if (!arrOperand1.length) {
      returnNum = 0;
    }
    return returnNum;
  }
